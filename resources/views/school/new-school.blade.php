            ...
            <div class="card height-auto false-height">
                <div class="card-body">
                    <form class="form-horizontal" enctype="multipart/form-data" method="POST"
                          action="{{ url('master/create-school') }}">
                        {{ csrf_field() }}
                        <div class="row">
                                ...
                                <div class="row">
                                    <div
                                        ...
                                        @if ($errors->has('school_name'))
                                            <span
                                                class="help-block"><strong>{{ $errors->first('school_name') }}</strong></span>
                                        @endif
                                    </div>
                                    <div
                                        ...
                                        @if ($errors->has('school_medium'))
                                            <span
                                                class="help-block"><strong>{{ $errors->first('school_medium') }}</strong></span>
                                        @endif
                                    </div>
                                    <div
                                        ...
                                        <input readonly="readonly" data-date-format="yyyy-mm-dd" id="birthday"
                                               value="{{old('school_established')}}" class="form-control date"
                                               name="school_established" placeholder="School Established" required
                                               autocomplete="off">
                                            ...
                                            @if (config('districts.districts'))
                                                @foreach ( config('districts.districts') as $district)
                                                    <option value="{{$district}}"
                                                            @if (old('district') == $district) selected @endif>{{ $district }}</option>
                                                @endforeach
                                            @endif
                                        </select>
                                    ...
                                    <div
                                        ...
                                        <select id="payment_type" class="form-control select2" name="payment_type"
                                            ...
                                            <option value="monthly"
                                                    @if (old('payment_type') == 'monthly') selected @endif>Monthly
                                            </option>
                                            <option value="per_student"
                                                    @if (old('payment_type') == 'per_student') selected @endif>Per
                                                Student
                                            </option>
                                        </select>
                                    ...
                                    <div
                                        ...
                                        <textarea id="school_about" class="form-control" rows="3" name="school_about"
                                                  ...
                                                  required>{{ old('school_about') }}</textarea>
                                        @if ($errors->has('school_about'))
                                            <span
                                                class="help-block"><strong>{{ $errors->first('school_about') }}</strong></span>
                                        @endif
                                    </div>
                                    <div
                            ...
                            <div class="col-md-6 col-sm-12 ">
                                <fieldset class="scheduler-border border border-secondary p-4">
                                    ...
                                    <div class="row">
                                        <div
                                            class="mb-4 col-md-6 {{ $errors->has('is_sms_enable') ? ' has-error' : '' }}">
                                            <label for="is_sms_enable" class="control-label">SMS Option <label
                                                    class="text-danger">*</label></label>
                                            <select id="is_sms_enable" class="form-control select2"
                                                    onchange="smsChargeFieldValidation()" name="is_sms_enable" required>
                                                <option value="" selected>SMS Option</option>
                                                <option value="1" @if (old('is_sms_enable') == 1) selected @endif>
                                                    Enable
                                                </option>
                                                <option value="0" @if (old('is_sms_enable') === 0) selected @endif>
                                                    Disable
                                                </option>
                                            </select>
                                        ...
                                        <div
                                            class="mb-4 col-md-12 {{ $errors->has('online_class_sms') ? ' has-error' : '' }}">
                                            <label for="online_class_sms" class="control-label">Online Class SMS</label>
                                            <select id="online_class_sms" class="form-control select2"
                                                    name="online_class_sms">
                                                <option value="" selected>SMS Option</option>
                                                <option value="1"
                                                        @if (old('online_class_sms') === true) selected @endif>Enable
                                                </option>
                                                <option value="0"
                                                        @if (old('online_class_sms') === false) selected @endif>Disable
                                                </option>
                                            </select>
                                        ...
                                        <div
                                            ...
                                            <select id="invoice_generation_date" class="form-control select2"
                                                ...
                                                @for($i = 1; $i <= 31; $i++)
                                                    <option value="{{$i}}"
                                                            @if(old('invoice_generation_date')== $i) selected @endif>{{ $i }}</option>
                                                @endfor
                                            </select>
                                        ...
                                        <div
                                            ...
                                            <select id="due_date" class="form-control select2" name="due_date">
                                                ...
                                                @for($i = 1; $i <= 31; $i++)
                                                    <option value="{{$i}}"
                                                            @if(old('due_date')== $i) selected @endif>{{ $i }}</option>
                                                @endfor
                                            </select>
                                        ...
                                        <div
                                            ...
                                            <input id="signup_date" data-date-format="yyyy-mm-dd" type="text"
                                                   ...
                                                   value="{{old('signup_date')}}" autocomplete="off">
                            ...
                            <div class="col-md-12 mt-4">
                                <button type="submit" id="registerBtn" class="button button--save float-right"><i
                                        class="fas fa-plus mr-2"></i>Create School
                                </button>
                            </div>
                        </div>
                    </form>
