        ...
        @foreach ($exams as $exam)
            <tr>
                ...
                <td>
                    ...
                    @if($exam->result_published != 1)
                        <span>
          <input type="submit" class="button button--save float-right" style="margin-left: 1%;" value="Save"
                 form="form{{$exam->id}}"/>
        </span>
                    @endif
                </td>
                <td>
                    <button class="btn btn-danger btn-lg" type="button" onclick="removeExam({{ $exam->id }})"><i
                            class="far fa-trash-alt"></i></button>
                    <form id="delete-form-{{ $exam->id }}" action="{{ url('admin/exams/remove', ['id' => $exam->id]) }}"
                    ...
                    <a href="{{ route('exams.attendees',['exam_id' => $exam->id]) }}"
                       class="btn btn-lg btn-primary text-white ml-3" title="Attendees"><i class="fas fa-users"></i></a>
                </td>
            </tr>
        @endforeach
        </tbody>
